...
class DataConverter(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._slicer = self._long_page_strategy.create_slicer(
            overlap=overlap, max_bpe_seq_length=self._max_bpe, tokenizer=self.tokenizer
        )
    ...
    def _create_input_and_target(
        self, bpe_tokens: List[str], seg_data: Dict[str, Any] = None
    ...
    def _salient_noise_mask(
        self,
        bpe_tokens: np.ndarray,
        seg_data: Dict[str, Any],
        min_salient_accepted_scalar=0.4,
        max_salient_accepted_scalar=1.2,
        balancing_coef=0.7,
        min_no_of_spans=5,
    ):
        """
        ...
        assert self.tokenizer.tokenize(" a")[0][0] in (
            "▁",
            "Ġ",
        ), "salient feature does not support this tokenizer"
        ...
        assert (
            text[starts[-2] : starts[-1]].strip()
            == bpe_tokens[-1].replace("▁", " ").replace("Ġ", " ").strip()
        ...
        category_sample = {
            k: round(len(v) ** balancing_coef) for k, v in doc_entities.items()
        }
        selected_entities = {
            k: sample(v, category_sample[k]) for k, v in doc_entities.items()
        }
        ...
        char_ranges = np.sort(
            [(ent.start_char, ent.end_char) for ent in flat_entities], 0
        )
        ...
        if span_lengths.sum() > noise_len_goal * max_salient_accepted_scalar:
            ...
            dropped_idx = []
            dropped_len = 0
            for idx, single_span_len in sorted(
                enumerate(span_lengths), key=lambda x: random()
            ):
    ...
    def create_example(
        self, doc: Doc2d, hard_token_limit: Optional[int] = None
    ) -> Example:
        ...
        tokens, token_ocr_ranges, seg_data, docid = (
            doc.tokens,
            doc.token_ocr_ranges,
            doc.seg_data,
            doc.docid,
        )
        ...
        page_same_size, page_most_dim = self._get_page_dim(
            seg_data["pages"]["org_bboxes"]
        )

        print(seg_data["pages"])
        for skey, seg in seg_data.items():
            if skey in ("images", "lazyimages"):
                continue
            seg["bboxes"] = self._convert_to_relative_page_pos(
                seg, skey, seg_data["pages"], page_same_size, page_most_dim
            )
        # exit()
        # add page numbers
    ...
    def create_spans(self, ex: Example) -> Sequence[Span]:
        ...
        for span_idx, (from_range, to_range) in enumerate(
            self._get_splits(ex.tokens, ex.seg_data)
        ):
        ...
        return [
            span for span in spans if span is not None and self._is_span_valid(span)
        ]

    def convert_span_to_feature(
        self, span: Span, seq_len: Optional[int] = None
    ) -> Feature:
        """Convert span to feature - input for the model."""

        seq_len = self._max_seq_length if seq_len is None else seq_len
        assert (
            len(span.tokens) <= seq_len
        ...
        label_ids = self._feature_empty_array(
            "lm_label_ids", seq_len, default=self._padding_idx
        )
    ...
    def __add_segment_2d_features(
        self, seg: Dict[str, Any], seq_len: int
    ) -> Dict[str, Any]:
        result = {}
        for el_key, el in seg.items():
            if el_key in FEAT_META.keys() and (
                "train_dtype" in FEAT_META[el_key] or el_key == "path"
            ):
    ...
    def adjust_bboxes_to_page_number(span: Span) -> Span:
        cum_height = 0
        for height, (start, end) in zip(
            span.seg_data["pages"]["bboxes"][:-1, 3],
            ...
            if "lines" in span.seg_data:
                ...
                span.seg_data["lines"]["bboxes"][
                    rng_start:rng_end, [1, 3]
                ] += cum_height

        return span

    def convert_spans_to_features(
        self, spans: Sequence[Span], seq_length: Optional[int] = None
    ) -> Sequence[Feature]:
        """Convert spans to features - inputs for the model"""
        return [self.convert_span_to_feature(span, seq_length) for span in spans]

    def _get_splits(
        self, bpe_tokens: Sequence[str], seg_data: Dict[str, Any]
    ...
    def _transform_to_bpe(
        ...
        if isinstance(self._tokenizer, PreTrainedTokenizerFast):
            bpe = [
                e.tokens
                for e in self._tokenizer.backend_tokenizer.encode_batch(
            ...
            bpe = [
                (
                    self._tokenizer.tokenize(tok, add_prefix_space=True)
                    if len(tok) > 0
                    else []
                )
                for tok in tokens
            ...
            bpe = [
                self._tokenizer.tokenize(tok) if len(tok) > 0 else [] for tok in tokens
            ]
        ...
        for tok_idx, (t, blst) in enumerate(zip(tokens, bpe)):
            ...
            if "tokens" in seg_data:
                bpe_bboxes.extend(
                    get_bpe_positions(
                        seg_data["tokens"]["org_bboxes"][tok_idx], bpe_lens
                    )
                )
            if use_ocr_ranges:
                cuml = [0] + list(accumulate(bpe_lens))
                trng = token_ocr_ranges[tok_idx]  # type: ignore
                bpe_token_ocr_ranges.extend(
                    (
                        min(trng[0] + cuml[li], trng[1]),  # type: ignore
        ...
        if bpe_token_ocr_ranges is not None:
            bpe_token_ocr_ranges = convert_to_np(
                bpe_token_ocr_ranges, "ocr_ranges"
            ).clip(0)

        # recalculate ranges in segments due to bpe, assign new bpe token boxes to dict item
        for skey, seg in seg_data.items():
            if skey == "tokens":
                seg_data["tokens"]["org_bboxes"] = convert_to_np(
                    bpe_bboxes, "org_bboxes"
                )
            elif skey in ("lines", "pages"):
                seg["ranges"] = self._recalculate_seg_ranges(
                    seg["ranges"], tok_bpe_map, org_tokens_idx
                )
    ...
    def _add_special_tokens(
        ...
        # add special tokens
        bpe_tokens = (
            [self.tokenizer.cls_token] + bpe_tokens + [self.tokenizer.sep_token]
        )
        ...
        for segkey, seg in seg_data.items():
            if segkey == "tokens":
                seg["bboxes"] = np.pad(
                    seg["bboxes"], ((1, 1), (0, 0)), "constant", constant_values=0
                )
                seg["org_bboxes"] = np.pad(
                    seg["org_bboxes"], ((1, 1), (0, 0)), "constant", constant_values=0
                )
    ...
    def __is_seg_valid(self, key: str, seg: Dict[str, Any], token_count: int):
        if key == "tokens":
            ...
            return (
                len(seg["ranges"]) == len(seg["bboxes"]) == len(seg["org_bboxes"])
                and len(seg["ranges"]) <= self._max_seq_length
            )
    ...
    @staticmethod
    def _convert_to_relative_page_pos(
        seg: Dict[str, Any],
        seg_key: str,
        page_seg: Dict[str, Any],
        page_same_size: bool,
        page_most_dim: Tuple[int, ...],
    ) -> np.ndarray:
        # positions - n x 4 array
        # page_positions = 1 x 4 array
        print("page_same_size", page_same_size)
        print(
            "page_most_dim", page_most_dim, page_most_dim, page_most_dim[2] * np.sqrt(2)
        )
        rel_pos = np.zeros_like(seg["org_bboxes"], dtype=float)
        rel_pos[:, 0] = seg["org_bboxes"][:, 0].clip(min=0, max=page_most_dim[2]) / (
            page_most_dim[2]
        )
        rel_pos[:, 2] = seg["org_bboxes"][:, 2].clip(min=0, max=page_most_dim[2]) / (
            page_most_dim[2]
        )
        rel_pos[:, 1] = seg["org_bboxes"][:, 1].clip(min=0, max=page_most_dim[3]) / (
            page_most_dim[3]
        )
        rel_pos[:, 3] = seg["org_bboxes"][:, 3].clip(min=0, max=page_most_dim[3]) / (
            page_most_dim[3]
        )

        print("rel_pos", rel_pos[0])
        ...
        if not page_same_size:
            for psize, prange in zip(page_seg["org_bboxes"], page_seg["ranges"]):
                ...
                print(idx, psize[2] * np.sqrt(2))
                rel_pos[idx, 0]
                rel_pos[idx, 0] = seg["org_bboxes"][idx, 0].clip(
                    min=0, max=psize[2]
                ) / (psize[2] * np.sqrt(2))
                rel_pos[idx, 2] = seg["org_bboxes"][idx, 2].clip(
                    min=0, max=psize[2]
                ) / (psize[2] * np.sqrt(2))
                rel_pos[idx, 1] = seg["org_bboxes"][idx, 1].clip(
                    min=0, max=psize[3]
                ) / (psize[2] * np.sqrt(2))
                rel_pos[idx, 3] = seg["org_bboxes"][idx, 3].clip(
                    min=0, max=psize[3]
                ) / (psize[2] * np.sqrt(2))
    ...
    @staticmethod
    def _recalculate_seg_ranges(
        seg_ranges: np.ndarray,
        tok_bpe_map: Sequence[Tuple[int, int]],
        org_tokens_idx: Sequence[int],
    ) -> np.ndarray:
        if len(tok_bpe_map) == 0:
            ...
            bpe_rng_np = np.stack(
                (
                    np.searchsorted(tok_idx, seg_ranges[:, 0], "left"),
    ...
    def _add_page_no(seg_data: Dict) -> Dict:
        ...
        if "pages" in seg_data:
            ...
            seg_data_["pages"]["ordinals"] = np.arange(
                seg_data["pages"]["ranges"].shape[0]
            )
    ...
    def _feature_empty_array(feature: str, seq_length: int, default=None) -> np.ndarray:
        ...
        return np.full(
            ([seq_length] + feat["dim"]) if feat["wide"] else feat["dim"],
            dtype=feat["dtype"],
            fill_value=default,
        )
    ...
    @staticmethod
    def _get_page_dim(
        pages_dim: Sequence[Sequence[int]],
