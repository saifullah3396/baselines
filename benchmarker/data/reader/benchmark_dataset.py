...
class BenchmarkDataset(Dataset):
    ...
    def __iter__(self) -> Iterator[Document]:
        ...
        docs_content_jsonl_path = (
            self.directory / self.split / "documents_content.jsonl"
        )
        with open(docs_jsonl_path) as docs_file, open(
            docs_content_jsonl_path
        ) as docs_content_file:
            for doc_line, doc_content in zip(docs_file, docs_content_file):
                ...
                if not tool2cf[self.ocr]["common_format"]["tokens"]:
                    logging.warning(
                        f'No tokens in common format for {doc_dict["name"]}. Skipping it'
                    )
                    continue
                ...
                img_dir = self.directory / "pdfs" / identifier.split(".pdf")[0]
                    ...
                    for i, value in enumerate(annotation["values"]):
                        if "children" in value:
                            ...
                            for child in value["children"]:
                                ...
                                annotations[child_question] += get_child_values(
                                    child["values"]
                                )
                        else:
